...
public class ResumenCompra extends javax.swing.JFrame implements TemporizadorObserver {
    ...
    /**
     * Constructor de la ventana ResumenCompra. Inicializa componentes, registra
     * el temporizador como observador y muestra el resumen del boleto actual
     * desde el contexto.
     */
    ...
    /**
     * Acción que se ejecuta al presionar el botón "Aceptar". Abre la ventana de
     * método de pago y cierra la actual.
     */
    ...
    /**
     * Muestra el resumen del boleto seleccionado en la interfaz. Incluye
     * origen, destino, camión, asientos, nombres de pasajeros, precio
     * individual, total y monedero generado (5% del total).
     *
     * @param boleto BoletoDTO con la información de la compra.
     */
    public void mostrarResumen(BoletoDTO boleto) {
        ...
        BigDecimal monederoRedondeado = new BigDecimal(monederoGenerado).setScale(2, RoundingMode.HALF_UP);

        lblPrecio.setText("$" + String.format("%.2f", precio));
        lblTotal.setText("$" + String.format("%.2f", total));
        lblMonedero.setText("$" + String.format("%.2f", monederoRedondeado));
    }

    /**
     * Se ejecuta cuando el tiempo del temporizador se agota. Muestra un mensaje
